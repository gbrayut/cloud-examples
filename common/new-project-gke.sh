...
gcloud compute networks subnets create gke-oregon-subnet \
...
gcloud compute networks subnets create gke-oregon-subnet2 \
    --network "projects/$DEVSHELL_PROJECT_ID/global/networks/gke-vpc" \
    --region us-west1 --range 10.30.236.0/22 \
    --secondary-range gkepods=10.112.0.0/13,gkeservices=10.65.0.0/16

gcloud compute networks subnets create gke-iowa-subnet \
    --network "projects/$DEVSHELL_PROJECT_ID/global/networks/gke-vpc" \
...
gcloud container --project=$DEVSHELL_PROJECT_ID node-pools create "spot-pool" --cluster "gke-iowa" --region "us-central1" \
  ...
  --spot --tags=tag1,tag2 --node-labels=example.com/fleet=spot --node-taints=example.com/testing=tainted:NoSchedule
