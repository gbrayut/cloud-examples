...
It make take 10-15 minutes for the load balancer to finish programming. You can check the status in the [Cloud Console](https://console.cloud.google.com/kubernetes/gateways) or using `kubectl describe gateway ....`
...
curl -sk --cert client-123.mtls.example.com.crt --key client-123.mtls.example.com.key --resolve example.com:443:$ALB_IP https://example.com | jq '.headers' | tee /tmp/headers.json
{
  "Accept": "*/*",
  "Host": "example.com",
...
# Check thumbprint on client cert
...
# Verify Leaf and Chain certificates
jq -r '."X-Mtls-Cert-Leaf"|split(":")[1]' /tmp/headers.json | basenc --base64 --decode | openssl asn1parse -inform DER
jq -r '."X-Mtls-Cert-Chain"|split(":")[1]' /tmp/headers.json | basenc --base64 --decode | openssl asn1parse -inform DER
```
...
```shell
# Regional GCLB will also needs proxy subnets:
...
kurl -sk --cert client-123.mtls.example.com.crt --key client-123.mtls.example.com.key --resolve example.com:443:$RLB_IP https://example.com | jq '.headers' | tee /tmp/headers.json
...
kurl -sk --cert client-123.mtls.example.com.crt --key client-123.mtls.example.com.key --resolve example.com:443:$ILB_IP https://example.com | jq '.headers' | tee /tmp/headers.json
```
